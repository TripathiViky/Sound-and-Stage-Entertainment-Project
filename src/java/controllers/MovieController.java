...
public class MovieController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String gnr=request.getParameter("txtgenre");
            String cast=request.getParameter("txtcast");
            models.MovieModel objm=new MovieModel();
            
            objm.setName(Mname);
            objm.setGenere(gnr);
            objm.setCasts(cast);
            String res = objm.insert();
            res+="<hr/>";
            res+=objm.getRecords();
            request.setAttribute("result", res);
            RequestDispatcher rd=request.getRequestDispatcher("admin/NewMovie2.jsp");
